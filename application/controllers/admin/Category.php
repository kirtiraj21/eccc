...
public function index()
  {
	    ...
	    $data["page"]="Category";
		$data["view"] = $this->Home_model->get_tbl_data(TBL_CATEGORY,array('status'=>1));
...
public function add()
 ...
 public function edit($cid)
  {   
      ...
      $rec = $this->Home_model->get_single_row(TBL_CATEGORY,array('id'=>$id));
        if($this->input->post('submit')){
             $this->form_validation->set_rules('name', 'Name', 'trim|required');
    ...
    //  if(count($catcheck) == 0){
            ...
            if($insert){
			    ...
			    redirect('admin/Category/edit/'.$cid);
			}else{
				$this->session->set_flashdata('errmessage', 'Some problem occured please try after some time');
			    redirect('admin/Category/edit/'.$cid);
			}
