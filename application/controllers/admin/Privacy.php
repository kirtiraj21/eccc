...
public function index()
  {
	    ...
	    $data["page"]="Privacy";
		$data["view"] = $this->Home_model->get_tbl_data(TBL_PRIVACY,array());
...
public function add()
 ...
 public function edit()
  {   
      ...
      $rec = $this->Home_model->get_single_row(TBL_PRIVACY,array());
        if($this->input->post('submit')){
		 ...
		 $datap = $this->db->select('*')->from('privacy')->get()->row();
  	       
			if($datap!=''){
           $insert = $this->Home_model->update(TBL_PRIVACY,$datap->id,$arr);	
			}else{
			   	$insert = $this->Home_model->insert(TBL_PRIVACY,$arr);	 
			}
            if($insert){
				$this->session->set_flashdata('message', 'Privacy has been updated successfully');
			    redirect('admin/Privacy/edit/'.$cid);
			}else{
				$this->session->set_flashdata('errmessage', 'Some problem occured please try after some time');
			    redirect('admin/Privacy/edit/'.$cid);
			}			
	    ...
	    $data["page"]="Edit Privacy";
		$data["records"] = $rec;
	    $this->load->view('admin/privacy_add',$data);
 }
